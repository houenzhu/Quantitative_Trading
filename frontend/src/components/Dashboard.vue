...
const handleSelectStock = async (item) => {
  ...
  if (item && item.code && item.name) {
    try {
      const res = await stockApi.addStock(item.code, item.name)
      if (res.code === 200 && res.data) {
        stockPool.value[item.code] = item.name
        ElMessage.success(`已添加 ${item.code} - ${item.name} 到股票池`)
      } else {
...
const handleRemoveStock = async (code) => {
  try {
    const res = await stockApi.removeStock(code)
    if (res.code === 200 && res.data) {
      delete stockPool.value[code]
      ElMessage.success(`已从股票池移除 ${code}`)
    } else {
      ElMessage.warning(res.message || '移除失败')
    }
  } catch (e) {
